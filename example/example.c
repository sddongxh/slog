...
int main() {
  ...
  slog_init("/tmp/example.log", nLogFlags, 0);
   ...
   */
  slog_print(SLOG_DEBUG, 0,
             "Old way printed debug message with our own new line character\n");

  /* Old way logging function with error tag and enabled new line from argument
   */
  slog_print(SLOG_ERROR, 1, "Old way printed error message with %s",
             "auto new line character");
  ...
  /* This will never be printed while SLOG_TRACE is disabled by slog_disable()
   * function */
  slog_trace("Test log with disabled tag");

  /* Enable file logger and color the whole log output instead of coloring only
   * tags*/
  slog_config_get(&slgCfg);
